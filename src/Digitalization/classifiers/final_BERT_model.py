...
project_id_training = pd.read_csv('/home/kno5cac/git/publicrd/src/Digitalization/classifiers/final_projectid_training.csv')
project_id_validation = pd.read_csv('/home/kno5cac/git/publicrd/src/Digitalization/classifiers/final_projectid_bert_validation.csv')
project_id_testing = pd.read_csv('/home/kno5cac/git/publicrd/src/Digitalization/classifiers/final_projectid_bert_testing.csv')

# convert to lists
project_id_training = list(project_id_training['0'])
project_id_validation = list(project_id_validation['0'])
project_id_testing = list(project_id_testing['0'])

df_train = df_lab[df_lab['PROJECT_ID'].isin(project_id_training)]
df_val = df_lab[df_lab['PROJECT_ID'].isin(project_id_validation)]
df_test = df_lab[df_lab['PROJECT_ID'].isin(project_id_testing)]
...
def extrapolate(model, test_data):
    ...
    with torch.no_grad():
        ...
        for test_input, test_label in test_dataloader:
              ...
              round = round + 1  
              print(round)
    results.to_csv('/home/kno5cac/git/publicrd/src/Digitalization/classifiers/final_BERTClassifier_results.csv')
